...
# -*- coding: utf-8 -*-
"""
v2 -> v4.1 semi-automatic tagger (REVIEW mode)
- symptoms (0~2) + behaviors (0~2) + severity
- Conservative rules; designed to reduce "missing" for common expressions.
- Writes incrementally; supports resume by skipping existing ids in output.

Key updates in v4.1 (based on your valid samples):
1) symptoms: improved recall for
   - 피로: "회복이 느리다", "컨디션 회복", "활력"
   - 햇빛/실내: supports "햇볕" variant and "햇볕을 거의 못 봤어요"
2) behaviors: added
   - 과일섭취부족
   - 유제품섭취부족
3) Policy: DO NOT expand symptom tag set for edema/tingling/appetite loss now.
   These may remain blank ([]) by design.

Usage:
  python tagger_v4_1_review.py --in data_v2/train_v2.jsonl --out data_v4/train_v4_1.jsonl --split train --review
  python tagger_v4_1_review.py --in data_v2/valid_v2.jsonl --out data_v4/valid_v4_1.jsonl --split valid --review
"""
...
TAG_RULES: Dict[str, List[re.Pattern]] = {
    "피로": _re(
        ...
        r"체력.*(떨어|부족|약)",
        r"활력(이)? 부족", r"활력",
        r"회복(이)? 느리", r"회복(이)? (쉽게 )?안", r"회복이 오래",
        r"컨디션.*회복"
    ),
    "수면": _re(
    ...
    "빈혈/어지럼": _re(
        ...
        # optional weak-signal: cold hands/feet (you can comment out if you want stricter precision)
        r"손발(이)? 차", r"손(이)? 차", r"발(이)? 차"
    ),
    "집중/인지": _re(
    ...
    "햇빛/실내": _re(
        r"실내", r"자외선",
        r"햇빛", r"햇빛.*(못|거의|잘)? ?(쬐|보|봤)", r"햇빛을 거의 못",
        r"햇볕", r"햇볕.*(못|거의|잘)? ?(쬐|보|봤)", r"햇볕을 거의 못",
...
def prioritize_symptoms(symptoms: List[str]) -> List[str]:
    # If 햇빛/실내 appears, keep it first (strong signal)
    if "햇빛/실내" in symptoms:
        symptoms = ["햇빛/실내"] + [t for t in symptoms if t != "햇빛/실내"]
    return symptoms[:2]
...
def review_loop(in_path: str, out_path: str, split: str):
    ...
    for line_no, obj in read_jsonl(in_path):
        ...
        proposed_sym = propose_from_rules(text, TAG_RULES, SYMPTOMS, cap=3)  # propose up to 3 then cap to 2 after prioritize
        proposed_sym = prioritize_symptoms(proposed_sym)  # final cap 2
