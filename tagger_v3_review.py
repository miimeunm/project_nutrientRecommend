...
TAG_RULES: Dict[str, List[re.Pattern]] = {
    ...
    "수면": _re(
        r"수면", r"잠(이)? 안", r"잠들기", r"자주 깨", r"뒤척", r"불면",
        r"숙면", r"개운(하지)? 않", r"새벽"
    ),
    "스트레스": _re(
    ...
    "면역": _re(
        r"면역", r"감기(가)? (자주|잦)", r"몸살", r"자주 아프", r"병치레", r"컨디션(이)? 자주 무너"
    ),
    "소화/속불편": _re(
    ...
    "근육/쥐": _re(
        r"쥐(가)? 나", r"경련", r"근육(이)? (뭉치|뻐근|당기)", r"근육통"
    ),
    "빈혈/어지럼": _re(
        r"빈혈", r"어지럽", r"현기증", r"핑(이)? 돌", r"창백", r"눈앞(이)? 깜깜"
    ),
    "집중/인지": _re(
        r"집중(이)? 안", r"멍(하)?", r"기억(이)? 안", r"건망증", r"머리(가)? (안|잘) 돌아", r"인지"
    ),
    "피부/손톱/머리": _re(
        r"피부", r"트러블", r"여드름", r"각질", r"건조", r"손톱", r"머리카락", r"탈모"
    ),
    "간/회식": _re(
        r"간(이)? (피곤|안 좋|걱정)", r"숙취", r"회식", r"과음", r"음주(가)? (잦|많)"
    ),
}
...
def propose_tags(text: str) -> List[str]:
    ...
    return [tag for tag, _ in hits[:2]]
